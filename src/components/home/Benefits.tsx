   ...
   return (
      <Container className="flex flex-wrap mb-20 lg:gap-10 lg:flex-nowrap ">
         <div className={`flex items-center justify-center w-full lg:w-1/2 ${props.imgPos === 'right' ? 'lg:order-1' : ''}`}>
            <div>
               <img src={data.image} width={521} height={521} alt="Benefits" className={'object-contain'} />
            </div>
         </div>
         ...
         <div className={`flex flex-wrap items-center w-full lg:w-1/2 ${data.imgPos === 'right' ? 'lg:justify-end' : ''}`}>
            <div>
               <div className="flex flex-col w-full mt-4">
                  <h3 className="max-w-2xl mt-3 text-3xl font-bold leading-snug tracking-tight lg:leading-tight lg:text-4xl">{data.title}</h3>

                  <p className="max-w-2xl py-4 text-lg leading-normal text-gray-500 dark:text-gray-400">{data.desc}</p>
               </div>
