...
function FaqItem({ item }: { item: FaqItem }) {
   ...
   return (
      <div className="mb-5">
         ...
         >
            <p className="text-lg font-medium text-gray-800 dark:text-gray-200">{item.question}</p>
            <div className="flex items-center justify-center w-8 h-8 border rounded-full border-gray-300 dark:border-gray-600">
               <svg
                  viewBox="0 0 24 24"
                  className={`w-3 text-gray-600 dark:text-gray-400 transform transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
               >
                  <polyline
                     fill="none"
                     stroke="currentColor"
